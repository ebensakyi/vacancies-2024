...
const Employment = ({ data }: any) => {
  ...
  const next = async () => {
    ...
    if (data?.employments?.response.length > 0) {
      if (data?.staffTypes.includes(1)) {
        await router.push(`/applicant/essay?next=true`);
        return;
      } else {
        if (data?.staffTypes.includes(1)) {
          await router.push(`/applicant/references?next=true`);
          return;
        }
      }
    }
              ...
              <div className="card">
                ...
                <div className="card-body">
                  <span className="badge bg-success " style={{ padding: 10 }}>
                    {" "}
                    Add all employments here. Order by most recent job. Check
                    the current job check box, if you are still at post. Click
                    on (+) to add a employment.
                  </span>
